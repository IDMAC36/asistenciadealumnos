...
@section('title', $student->name . ' - Asistencia QR')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 flex flex-col items-center">
            ...
            <p class="text-xs text-gray-400 mt-3 font-mono">{{ $student->qr_code }}</p>
        ...
        <div class="border-t border-gray-100 px-6 py-4">
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->created_at->format('d/m/Y H:i') }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500 uppercase tracking-wider">Asistencias totales</dt>
                    <dd class="text-sm font-medium text-gray-900 mt-1">{{ $student->attendances()->count() }}</dd>
                </div>
            </dl>
        </div>
...
function downloadQrAsPng() {
    ...
    const img = new Image();
    img.onload = function () {
        // Create canvas with white background + QR + student name
        const padding = 40;
        const textHeight = 60;
        ...
        // Draw student name below
        ctx.fillStyle = '#1b1b18';
        ctx.font = 'bold 18px Arial, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(@json($student->name), canvas.width / 2, 400 + padding + 28);

        // Draw grado
        ctx.fillStyle = '#706f6c';
        ctx.font = '14px Arial, sans-serif';
        ctx.fillText(@json($student->grado), canvas.width / 2, 400 + padding + 50);

        // Trigger download
        const link = document.createElement('a');
        link.download = @json('qr-' . Str::slug($student->name) . '.png');
