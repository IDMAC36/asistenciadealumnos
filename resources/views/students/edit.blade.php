        ...
        <form action="{{ route('students.update', $student) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text"
                       ...
                       id="codigo_personal"
                       value="{{ old('codigo_personal', $student->codigo_personal) }}"
                       required
            ...
            <div>
                ...
                <select name="plan"
                    ...
                    <option value="plan_diario" {{ old('plan', $student->plan) === 'plan_diario' ? 'selected' : '' }}>Plan Diario</option>
                    <option value="plan_fin_de_semana" {{ old('plan', $student->plan) === 'plan_fin_de_semana' ? 'selected' : '' }}>Plan Fin de Semana</option>
                </select>
                @error('plan')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
