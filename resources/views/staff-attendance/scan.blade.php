...
@section('title', 'Escanear QR Personal - Asistencia QR')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('staff-attendance.index') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium transition-colors">
            ← Volver a asistencia personal
        </a>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-green-600 px-6 py-5">
            <h1 class="text-xl font-bold text-white">📷 Escanear QR del Personal</h1>
            <p class="text-green-200 text-sm mt-1">Apunta la cámara al código QR del miembro del personal</p>
        </div>
        ...
        <div class="p-6">
            ...
            <div id="success-overlay" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div class="bg-white rounded-2xl shadow-2xl p-8 mx-4 max-w-sm w-full text-center transform transition-all">
                    ...
                    <p id="success-staff-name" class="text-lg font-semibold text-gray-800"></p>
                    <p id="success-staff-time" class="text-sm text-gray-500 mt-1"></p>
            ...
            <div id="already-overlay" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div class="bg-white rounded-2xl shadow-2xl p-8 mx-4 max-w-sm w-full text-center transform transition-all">
                    ...
                    <p id="already-staff-name" class="text-lg font-semibold text-gray-800"></p>
                    <p id="already-staff-time" class="text-sm text-gray-500 mt-1"></p>
                    <div class="mt-6 h-1 bg-gray-200 rounded-full overflow-hidden">
...
<script>
    ...
    function onScanSuccess(decodedText, decodedResult) {
        ...
        fetch('{{ route("staff-attendance.mark") }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                addToLog(data.staff.name, data.staff.time, true);
                showSuccessOverlay(data.staff.name, data.staff.time);
                ...
                showAlreadyOverlay(data.staff.name, data.staff.time);
            } else {
                messageDiv.textContent = data.message;
    ...
    function showSuccessOverlay(name, time) {
        ...
        document.getElementById('success-staff-name').textContent = name;
        document.getElementById('success-staff-time').textContent = 'Hora: ' + time;
    ...
    function showAlreadyOverlay(name, time) {
        ...
        document.getElementById('already-staff-name').textContent = name;
        document.getElementById('already-staff-time').textContent = 'Registrado a las ' + time;
