...
@section('title', 'Reporte Mensual - Personal')
...
<div class="max-w-full mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">📋 Reporte Mensual — Personal</h1>
            <p class="text-gray-500 text-sm mt-1">Asistencia general agrupada por rol</p>
        </div>
        <div class="mt-4 sm:mt-0 flex items-center gap-2">
            <a href="{{ route('staff-attendance.monthly-report', ['month' => $prevMonth]) }}"
            ...
            <a href="{{ route('staff-attendance.monthly-report', ['month' => $nextMonth]) }}"
               class="px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm rounded-lg transition-colors">
                Siguiente →
            </a>
            <a href="{{ route('staff-attendance.monthly-report.export', ['month' => $month]) }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ route('staff-attendance.monthly-report') }}" class="flex flex-col sm:flex-row items-end gap-3">
    ...
    {{-- Tablas por rol --}}
    @forelse($reportData as $role => $members)
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
        <div class="bg-indigo-600 px-5 py-3">
            <h2 class="text-white font-bold text-sm">👔 {{ $role }}</h2>
        </div>
    ...
    @empty
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p class="text-gray-500 text-sm">Registra miembros del personal para ver el reporte mensual.</p>
        </div>
    @endforelse
</div>
@endsection
